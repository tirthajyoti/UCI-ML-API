...
def read_dataset_table(url="https://archive.ics.uci.edu/ml/datasets.php",msg_flag=True):
	...
	# Assign column names
    df.columns=['Name','Data Types','Default Task','Attribute Types','Number of Instances','Number of Attributes','Year']
...
def build_local_table(filename=None,msg_flag=True):
    ...
    try:
        ...
        print("Sorry, could not create the CSV table. Please make sure to close an already opened file, \
...
#======================================================================================
def build_dataset_dictionary(url='https://archive.ics.uci.edu/ml/datasets.html?format=&task=&att=&area=&numAtt=&numIns=&type=&sort=nameUp&view=list',msg_flag=True):
    """
...
def return_abstract(name,local_database=None,msg_flag=False):
    ...
    for r in range(nrows):
        if name in df.iloc[r]['Name']:
            found+=1
            abstracts.append(df.iloc[r]['Name']+": "+df.iloc[r]['Abstract']+\
                             ". For more info, visit this link: "\
                             +"https://archive.ics.uci.edu/ml/datasets/"+df.iloc[r]['Identifier string'])
    if found==0:
...
#=======================================================
def download_datasets_size(size='Small',local_database=None,local_table=None,msg_flag=False,download_flag=True):
    """
    ...
    download_all_from_dataframe(df_filter,msg_flag=msg_flag,download_flag=download_flag)    

#===========================================================================
# User API Function to download datasets based on the machine learning task
#===========================================================================
def download_datasets_task(task='Classification',local_database=None,local_table=None,msg_flag=False,download_flag=True):
    """
    Downloads all datasets which satisfy the size criteria.
